...
class PersonDetector:
    ...
    def _compute_weight(self, pix_coord: np.ndarray):
        cxy = np.array([self.cam.cx, self.cam.cy])
    ...
    def process_frame(
        ...
        for box in result.boxes:
            ...
            vec_world = (rotation @ self.cam.reproject(vec).reshape(-1, 1)).flatten()
            ...
            if enable_localization and translation is not None and rotation is not None:
                ...
                world_coord, cluster_id = self.tracker.track(
                    pixel_coord, weight, translation, rotation, crop[None,...], toGPS, self.zone
                )
                ...
                if self.best_weight.get(cluster_id) is None:
                    self.best_weight[cluster_id] = weight
                    view_point = world_coord + 2 * vec_world
                    self.view_points[cluster_id] = view_point
                elif weight > self.best_weight[cluster_id]:
                    view_point = world_coord + 2 * vec_world
                    self.view_points[cluster_id] = view_point
    ...
    def get_valid_cluster_info(self) -> Dict:
        """Get information about current clusters."""
        return self.tracker.clusterer.get_valid_cluster_info()
