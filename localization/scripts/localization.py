...
class Clusterer:
    ...
    def __init__(
        ...
        Args:
            ...
            min_points_threshold (int): Minimum number of points required for a cluster to be considered valid.
        """
        self.distance_threshold = distance_threshold
        self.method = method
        self.min_points_threshold = min_points_threshold
        self.clusters = {}  # {cluster_id: [raw_detections, ...], ...}
        self.cluster_centroids = {}  # {cluster_id: cluster_centroid, ...}
        self.cluster_weights = {}
        self.cluster_crops = {}  # {cluster_id: cluster_crop, ...}
        self.next_cluster_id = 0


    def get_valid_clusters(self) -> dict:
        """Get only clusters that meet the minimum points threshold.
    ...
    def add_point(self, point: np.ndarray, weight: float, crop: np.ndarray) -> int:
        """Add a new point and return its cluster assignment.
        
        Args:
            point (np.ndarray): 3D coordinate point (shape: (3,))
            crop (np.ndarray): The cropped frame of the detection.
        ...
        if len(self.clusters) == 0:
            ...
            cluster_id = self._create_new_cluster(point, weight)
        else:
            ...
            if cluster_id is None:
                ...
                self._add_to_cluster(cluster_id, point, weight)
        
        self.cluster_crops[cluster_id] = cv2.resize(crop, (128, 128))
                
        return cluster_id
    
    
    def add_batch(self, points: np.ndarray, weight: float, crops: np.ndarray) -> List[int]:
        """Add multiple points at once.
            
        Args:
            points (np.ndarray): Array of 3D points (shape: (N, 3))
            crops (np.ndarray): The cropped frame of the detection.
            
        Returns:
            List[int] : cluster IDs for each point
        """
        cluster_ids = []
        for point, crop in zip(points, crops):
            cluster_ids.append(self.add_point(point, weight, crop))
        return cluster_ids
    ...
    def _create_new_cluster(self, point: np.ndarray, weight: float) -> int:
        ...
        self.cluster_weights[cluster_id] = weight
        
        return cluster_id


    def _add_to_cluster(self, cluster_id: int, point: np.ndarray, weight: float):
        ...
        self.cluster_centroids[cluster_id] = (old_centroid * self.cluster_weights[cluster_id] + point * weight) / (self.cluster_weights[cluster_id] + weight)
        self.cluster_weights[cluster_id] += weight
    ...
    def get_cluster_info(self) -> dict:
        ...
        for cluster_id in self.clusters:
            ...
            info[cluster_id] = {
                ...
                'crop': self.cluster_crops[cluster_id],
    ...
    def get_valid_cluster_info(self) -> dict:
        """Get information about valid clusters only (those meeting min_points_threshold).
        """
        info = {}
        for cluster_id in self.clusters:
            points = np.array(self.clusters[cluster_id])
            # Only include clusters that meet minimum threshold
            if len(points) >= self.min_points_threshold:
                info[cluster_id] = {
                    'num_points': len(points),
                    'centroid': self.cluster_centroids[cluster_id],
...
class Tracker:
    def __init__(
        ...
        self.clusterer = Clusterer(distance_threshold, method, min_points_threshold)
    ...
    def _compute_weight(self, pix_coord: np.ndarray):
        cxy = np.array([self.camera.cx, self.camera.cy])
        weight = 1. - (np.linalg.norm(pix_coord - cxy) / np.linalg.norm(cxy)) ** 0.5
        return weight
    ...
    def track(
        ...
        # weight = self._compute_weight(pixel_coord)
        cluster_ids = self.clusterer.add_batch(coord, weight, crops)
